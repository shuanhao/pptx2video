...
最後更新 `2026-08-05`　目前版本 `v0.8.0`　後端測試 `167 passed`
...
| 06 | **一次到底 / 分段輸出** | 以上步驟可以拆開一步步跑，方便個別除錯，也可以合併成單一指令一次完成；影片太長時，另有工具可依換頁邊界切成多段，方便觀看與分享。 |
...
系統不是用單純的字數上限，而是用**顯示寬度**：中文全形字算 2、英文數字等半形字算 1，預設每行上限是 32（相當於 16 個中文全形字）—— 這樣設計是因為中英文混排時，如果只算字數，一行英文跟一行中文讀起來的「份量」會差很多，用顯示寬度換算後兩者比較接近。
